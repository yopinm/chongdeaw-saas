...
- `app/store/[slug]/menu/page.tsx` — Server Component, fetch by slug → store_id
- `app/store/[slug]/menu/AvailabilityToggle.tsx` — Client toggle
- `app/api/menu/[slug]/toggle/route.ts` — POST verify store_id จาก JWT
- `scripts/seed-cloud.ts` — seed stores/categories/products ขึ้น cloud
- URL: `/store/chongdeaw-1/menu` ✅ แสดงเมนูได้แล้ว
`commit: chore: add scripts/seed-cloud.ts`
